...
of {\tt \small  apertium-nn-nb}, a machine translation system between Nynorsk
...
\citep{armentano2006ops}. In the following section we give an overview of
...
pipeline machine translation system. Dictionaries written in XML are
...
using a Constraint Grammar (CG) module\footnote{Using {\tt \small  VISL CG-3},
...
ambiguously tagged input (e.g. the English word `read' tagged both as a
...
iterations) was trained on the Nynorsk and Bokmål Wikipedias using the
{\tt \small apertium-tagger}. Although the Apertium toolset allows for
...
machine translation, and improvements to the CG could be of benefit to
others using the OBT. When disambiguating for machine translation, it
...
gathered with the {\tt \small  bitextor} web crawler tool
...
We next ran {\tt \small  Giza++} \citep{och2003scv} on a cleaned version of the KDE4
...
{\tt \small  ReTraTos} \citep{caseli2006aib}, which extracts both phrases and
...
which was automatically crawled and sentence aligned with the {\tt \small  bitextor}
tool mentioned above.

The {\tt \small  ReTraTos} method gave translations for about 3500 entries
...
(e.g.  changing directional constraints or morphological tags) or
...
\label{sec:WER}
We also tested the Word Error Rate (WER)\footnote{Using the tool {\tt \small 
...
%\footnote{WER for non-post-edited reference translations will of course be higher.}
...
\begin{table}[htdp]
  \caption{{\sc Bleu} score (two reference translations), UWR and WER (for the \emph{O}riginal and \emph{W}ikipedia references) for Apertium and Nyno}
\begin{center}
...
operative general-purpose Machine Translation systems between 
Scandinavian languages are rule-based, and most of them are powered by CG. 
Being open source, Apertium broadens the Scandinavian MT horizon.

%As the first open source RBMT platform, Apertium broadens the horizon of Scandinavian MT.

\section*{Acknowledgements}
[Anonymised]
\comment{Development was funded as part of the Google Summer of
