...
\newcommand{\comment}[1]{\textbf{SKRIV} #1}
...
\comment{ yes we want it to be good enough for dissemination/post-editing, not just assimilation/gisting}
...
\comment{ more? something like ``CG's have shown 94\%  P/R \citet{someone}'' ?}
...
\comment{ \texttt{texttt} for tags? or something else?\\}
...
gathered with the \texttt{bitextor} web crawler tool
...
} tool \texttt{poswap} to turn the English-Nynorsk and English-Bokmål
.po-files into Nynorsk-Bokmål files, then we ran
\texttt{poterminology}, a terminology extraction tool which gathers
...
We next ran \texttt{Giza++} \citep{och2003scv} on a cleaned version of the KDE4
...
\texttt{ReTraTos} \citep{caseli2006aib}, which extracts both phrases and
...
which was automatically crawled and sentence aligned with the \texttt{bitextor}
tool mentioned above.

The \texttt{ReTraTos} method gave translations for about 3500 entries which
...
agreement operations. Although Bokmål allows definite determiners with
indefinite nouns, Nynorsk does not; this is also handled with a macro.
...
dictionaries\footnote{Since the dictionaries are \emph{consistent}, we
  also have translations for all analysed words.}. Testing on Nynorsk
Wikipedia (5116174 words) and Bokmål Wikipedia (27529115 words), we
have 89.6\% and 88.2\% naïve coverage, respectively. On a 7019526 word
corpus of Bokmål newspaper texts, the coverage is 91.8\%.
...
\comment{ apertium-nn-nb may thus be used for dissemination (is
...
indefinite form, 98 out of 100 randomly selected compounds were
...
\ref{pass-c} above. We are also considering how this syntactic
...
``Well-written'' Nynorsk contains many such multi-word expressions,
but for now, without knowing where the relevant phrase ends, we take
the safe route and try to find non-discontinuous translations.

\comment{ noko om at nn-nb.po kan nyttast som TMX med Apertium?}

\comment {On the Scandinavian language group, and expanding it for Apertium:
...
\comment{ reporting CG errors which were not
  related
...
and constant help throughout, also to Jimmy O'Regan and the other
Apertium contributors. Thanks to Wikipedia contributors for reference
