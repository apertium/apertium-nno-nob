...
want the data from {\tt \small  apertium-nn-nb} to be useful in creating new
...
sets. To exemplify the latter: the OBT uses the tags {\tt \small 
  subst.appell} and {\tt \small  subst.prop} where Apertium uses {\tt \small  n} and
{\tt \small  np} respectively, so rules working on the single tag {\tt \small  subst}
were changed to work on the set consisting of the tags {\tt \small  n} and
{\tt \small  np}. Most of this conversion was done using simple shell scripts.
...
Wikipedias using the {\tt \small  apertium-tagger}. Although there is the
...
words) from Bokmål to Nynorsk in both Apertium and Nyno\footnote{The
  ...
  eg. ``radical'' versus ``conservative'' Nynorsk, whereas {\tt \small 
