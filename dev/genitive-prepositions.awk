...
BEGIN{
    ...
    bi_head_factor = 0.1 # head+pr has to be this much more frequent than head+til for making bigram gen rules
...
END{
    ...
    for(head in bi) for(pr in bi[head]) if(pr != "til") {
                for(obj in bi[head][pr]) {
                    ...
                    nno_bi_is_max = bimax[head,obj] > bi_factor  * bi[head]["til"][obj] && bimaxpr[head,obj] == pr && uni[head][pr] > bi_head_factor * uni[head]["til"]
